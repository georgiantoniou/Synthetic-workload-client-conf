...
echo "EXP: Client default arrival + server timer exp $runs $duration HOME_DIR=$HOME_DIR QPS=$QPS SERVICE_RATE=$SERVICE_RATE CONNECTIONS=$CONNECTIONS THREADS=$THREADS"
...
do
    ...
    do
        ...
        mv $HOME_DIR/timestamps_* "$EXP_DIR/run-$i-service-$service/"
